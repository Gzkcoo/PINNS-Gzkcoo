...
if __name__ == '__main__':
    ...
    u_sin = np.tile(u_sin, (100, 1))
    y_g = np.linspace(0, 1, num=100)[:, None]
    test = torch.from_numpy(np.hstack((u_sin, y_g)))  # 100*101
    ...
    u_g = copy_model(test) # 100*1

    # 画图比较
    fig, ax = plt.subplots()
    ax.plot(y_g, func(y_g), 'b', label='1-cosx')
    ax.plot(y_g, u_g.detach().numpy(), 'r', label='deeponet')
